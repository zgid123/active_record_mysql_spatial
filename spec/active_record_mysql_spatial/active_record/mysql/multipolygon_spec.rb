...
describe ActiveRecordMysqlSpatial::ActiveRecord::MySQL::Multipolygon do
  include_context :spatial_data

  context 'cast valid params' do
    %i[hash array].each do |type|
      context "params as #{type}" do
        subject { described_class.new.send(:cast_value, send("multipolygon#{type == :hash ? '' : '_array'}_params")) }

        it 'returns multipolygon' do
          expect(subject.type).to eq(:multipolygon)
          expect(subject.to_sql).to eq('MultiPolygon(((0.0 0.0, 10.0 0.0, 10.0 10.0, 0.0 10.0, 0.0 0.0)), ((5.0 5.0, 7.0 5.0, 7.0 7.0, 5.0 7.0, 5.0 5.0)))')
          ...
          expect(subject.items).to eq(
            [
              ActiveRecordMysqlSpatial::ActiveRecord::MySQL::Polygon.new.send(
                :cast_value,
                coordinates: [
                  [
                    { x: 0.0, y: 0.0 },
