...
describe ActiveRecordMysqlSpatial::ActiveRecord::MySQL::Linestring do
  include_context :spatial_data

  context 'cast valid params' do
    %i[hash array].each do |type|
      context "params as #{type}" do
        subject { described_class.new.send(:cast_value, send("linestring#{type == :hash ? '' : '_array'}_params")) }

        it 'returns linestring' do
          expect(subject.type).to eq(:linestring)
          expect(subject.to_sql).to eq('LineString(1 2, 2 3)')
